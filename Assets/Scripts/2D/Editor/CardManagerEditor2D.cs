...
[CustomEditor(typeof(CardManger2D))]

public class CardManagerEditor2D : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        float temp = _width.intValue * (float)_height.intValue / 2;
        _pairAmount.intValue = (int)System.Math.Ceiling(temp);

        if (_pairAmount.intValue > spriteAmount)
        {
            EditorGUILayout.HelpBox("To many pairs, not enough sprites !", MessageType.Error);
        }
